...
\begin{frame}[t]
	...
	\begin{columns}[t]
		...
		\begin{column}{\colwidth}
				...
				Based on stress driven model, nonlocal elastic curvatures $\chi(x)$ are the output of a spatial convolution between a kernel function $\phi$ and the bending moment $M(t)$:
				\begin{equation}
					\chi(x)=\int_{0}^{L}\phi(x-\overline{x},c)\frac{M(t)}{EI}dt 
				\end{equation}
				\begin{itemize}
					\item [$\square$] Leads to a sixth order governing differential equation.
					\item [$\square$] EFG method rely of scattered nodes and moving least square (MLS) approximations for constructing shape functions.
			...
			\begin{defbox}{Numerical Implementation}{}
				\begin{itemize}
					...
					\item [$\square$] The method results in a fully populated stiffness matrix, requiring more computational effort than traditional FEM but offers a more robust tool for nonlocal analysis.
				\end{itemize}
			...
			\end{defbox}
				\begin{defbox}{Validation and Parametric Study}{}
					...
					\begin{itemize}
					\item [$\square$] The SD-EFGM formulation was validated against experimental results from cantilever microbeam deflection tests.
